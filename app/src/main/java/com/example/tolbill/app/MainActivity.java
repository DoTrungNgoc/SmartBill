...
import com.example.tolbill.lib.RealPathUtil;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int IMAGE_PICK_GALLERY_CODE = 1200;
...
//        mResultEt = findViewById(R.id.resultEt);
    ...
    private void callApiUpload() {
        String realPath = RealPathUtil.getRealPath(this,result_uri);
        File file = new File(realPath);
        RequestBody requestImg = RequestBody.create(MediaType.parse("multipart/form-data"),file);
        MultipartBody.Part mutiPartImg = MultipartBody.Part.createFormData("image",file.getName(), requestImg);

        apiService.uploadAttachment(jwt,mutiPartImg).enqueue(new Callback<UploadResult>() {
            @Override
            public void onResponse(Call<UploadResult> call, Response<UploadResult> response) {
                mProgressDialog.dismiss();
                UploadResult rs = response.body();
                if (rs!=null){
                    Bill bill = rs.getBill();
                    startActivity(new Intent(MainActivity.this, Activity_update.class));
                }
            }
